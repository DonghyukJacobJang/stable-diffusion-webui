...
sudo apt-get update
sudo apt install wget git python3 python3-venv python3-pip build-essential net-tools awscli -y
...
# install Deforum 
git clone https://github.com/deforum-art/deforum-for-automatic1111-webui stable-diffusion-webui/extensions/deforum
...
# download mdjrny-v4 model
wget https://huggingface.co/prompthero/openjourney/resolve/main/mdjrny-v4.ckpt
mv mdjrny-v4.ckpt stable-diffusion-webui/models/Stable-diffusion/mdjrny-v4.ckpt

# download realistic vision
wget 'https://civitai.com/api/download/models/29460?type=Model&format=SafeTensor'
mv '29460?type=Model&format=SafeTensor' stable-diffusion-webui/models/Stable-diffusion/realisticVisionV20_v20.safetensors
...
# start the server as user 'ubuntu' and jupyter
sudo -u ubuntu nohup bash stable-diffusion-webui/webui.sh --listen > log.txt & 
sudo -u ubuntu nohup bash stable-diffusion-webui/setup-jupyter.sh
